...
func pclntabTextStart(data []byte, order binary.ByteOrder) (uint64, error) {
	...
	textStartOffset := 8 + 2*int(ptrSize)
	if len(data) < textStartOffset+int(ptrSize) {
...
func goTextStart(f *safeelf.File, textSection *elf.Section, pclntabData []byte) (uint64, error) {
	...
	for _, sectionName := range []string{".symtab", ".dynsym"} {
		...
		if err != nil {
			return 0, fmt.Errorf("reading %s: %w", sectionName, err)
		}
	...
	if dynamicSymbols, err := f.DynamicSymbols(); err == nil {
